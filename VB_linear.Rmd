...
## The codes for VB_linear_ARD{.tabset}
VARIATIONAL BAYESIAN INFERENCE FOR LINEAR REGRESSION with automatic relevance determination.(DRUGOWITSCH, 2013)
...
w <- rbind(matrix(rnorm(D_eff),ncol = 1), matrix(rep(0,D-D_eff),ncol = 1))
X <- matrix(runif(N*D),ncol = D)-0.5
X_test <- matrix(runif(N_test*D),ncol = D)-0.5
y <- X%*%w + matrix(rnorm(N), ncol = 1)
y_test <- X_test%*%w + matrix(rnorm(N_test), ncol = 1)
```

### model fit
```{r}
model_arc <- vb_linear_arc(X,y)

pred <- vb_linear_pred(X_test, w=model_arc$w,
              V = model_arc$V, an = model_arc$an, bn =model_arc$bn)
## the MSE
mean((y_test-pred$fit[,1])^2)

plot(w, model_arc$w, col="blue", pch=16, xlim = c(-4,4), 
     ylim = c(-4,4), xlab = "w", ylab = "w_vb")
lines(c(-4,4),c(-4,4), lty=3)

plot(y_test, pred$fit[,1], col="blue", pch=17, xlim = c(-10,10), 
     ylim = c(-10,10), xlab = "y_test", ylab = "y_test_vb")
